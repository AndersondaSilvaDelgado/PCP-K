...
class MovEquipResidenciaRepositoryImpl @Inject constructor(
    ...
    override suspend fun listMovEquipResidenciaStarted(): List<MovEquipResidencia> {
        return movEquipResidenciaDatasourceRoom.listMovEquipResidenciaStarted()
            .map { it.modelRoomToMovEquipResidencia() }
    }

    override suspend fun listMovEquipResidenciaStartedClosed(): List<MovEquipResidencia> {
        return movEquipResidenciaDatasourceRoom.listMovEquipResidenciaStartedClosed()
            .map { it.modelRoomToMovEquipResidencia() }
    }
    ...
    override suspend fun saveMovEquipResidencia(
        ...
        try {
            val movEquipResidenciaRoomModel =
                movEquipResidencia.entityToMovEquipResidenciaRoomModel(matricVigia, idLocal)
            if (!movEquipResidenciaDatasourceRoom.insertMovEquipResidenciaClose(
                    movEquipResidenciaRoomModel
                )
            ) return 0L
            if (!movEquipResidenciaSharedPreferencesDatasource.clearMovEquipResidencia()) return 0L
